...
class ApplyJob extends StatefulWidget {
  ...
  const ApplyJob(
      {Key? key, required this.id, required this.company,required this.job_id, required this.company_id, required this.user_name, required this.contact_number})
      : super(key: key);
...
class _ApplyJobState extends State<ApplyJob> {
  ...
  void initState() {
    ...
    FirebaseFirestore.instance
        ...
        .where('jobDocId', isEqualTo: widget.id)
        .get()
        .then((value) => {
              value.docs.map(
                (result) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
          stream: FirebaseFirestore.instance.collection('jobs').doc(widget.id).snapshots(includeMetadataChanges: true),
          builder:
                  ...
                  children: [
                              ...
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    ...
                                    //const SizedBox(height: 4),
                                    Text(
                                      data['city'] +',' + data['country'].toString().split(' ').elementAt(0),
                    ...
                    ListView(
                      ...
                      children: data['role']
                          .toString()
                          .split('.')
                          .map((String text) => bulletText(text))
                          .toList(),
                    ),
                    SizedBox(height: 15),
                    ElevatedButton(
  ...
  Future<String?> favourite(String UserKey) async
  {
    try
    {
      firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("favourite").doc(widget.id).set(
          {
      ...
      {
        firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("favourite").doc(widget.id).get().then((value){
  ...
  Future<String?> get_favourite() async
  {
    try
    {
      firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("favourite").get().then((value)
      {
        for(int i=0; i<value.docs.length; i++)
        {
          try
          {
            firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("favourite").doc(value.docs[i].id.toString()).get().then((value){
              ...
              if(h != null)
              {
                ...
                developer.log("my data g       "+ h.toString()+"    "+ h['job_id'].toString());
              }
            }).whenComplete(()
            {
  ...
  Future<String?> apply_job() async
  {
    try
    {
      firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("Applied_Jobs").doc(widget.id).set(
          {
      ...
      {
        firestoreInstance.collection("users").doc(widget.company_id).collection("Applied_user").doc(widget.id).set(
            {
              "job_id":widget.id,
              "job_title":jobTitle,
              "company_name":widget.company,
  ...
  Future<String?> get_applied_jobs() async
  {
    try
    {
      firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("Applied_Jobs").get().then((value)
      {
        for(int i=0; i<value.docs.length; i++)
        {
          try
          {
            firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("Applied_Jobs").doc(value.docs[i].id.toString()).get().then((value){
              Map<String,dynamic>? h = value.data();
              if(h != null)
              {
                if(widget.id == h['job_id'])
  ...
  Future<String?> get_profile_data() async
  {
    try
    {
      firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).get().then((value)
      {
        Map<String,dynamic>? h = value.data();
        ...
        developer.log("my data g       "+ h.toString()+"    "+ h!['name'].toString());
      });
    }
    catch(e)
    {
