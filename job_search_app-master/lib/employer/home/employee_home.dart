...
class _EmployeeHomeScreenState extends State<EmployeeHomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: customAppBarEmployer(context, 'Home'),
      body: Container(
  ...
  get_scheduled_user() async
  {
    ...
    try
    {
      firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("scheduled_interview").get().then((value){
        ...
        for(int i=0; i<value.docs.length; i++)
        {
          try
          {
            ...
            firestoreInstance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("scheduled_interview").doc(value.docs[i].id.toString()).get().then((value){
              Map<String,dynamic>? h = value.data();
              if(h != null)
              {
                _scheduledInterviewResponse = scheduledInterviewResponse.fromJson(h);
